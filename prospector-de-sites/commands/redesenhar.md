...
Apresente os arquivos criados ao usuário (páginas + editores + `comparar.html`) com um resumo de 1 linha por cliente (o que melhorou). Oriente: abrir `comparar.html` para ver antes/depois lado a lado, `[slug]-editor.html` para editar. Sugira `/publicar` para subir no GitHub + Vercel.
